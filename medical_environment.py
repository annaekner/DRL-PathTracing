...
# Their DataLoader file: src/medical.py

from data_loader import DataPancreas
...
# TODO: Update reward function (calcDistance and _calc_reward methods)
# TODO: Update action space from 6 -> 26 (action_space attribute, get_action_meanings and step methods)
# TODO: Update initialization of starting point from random -> F1 (new_random_game method)

# TODO: What are lives? Should this implementation be kept?
# TODO: This implementation is of landmark detection. How to do path tracing? Store trajectory!

class MedicalImagesEnv(gym.Env):
    """
    Class that provides 3D medical image environment.
    At each time-step, the agent chooses an action, and the environment returns an observation and a reward.
    """
    def __init__(self, directory=None, viz=False, task=False, files_list=None,
                 file_type="pancreas", landmark_ids=None,
        ...
        """
        Inputs:
        :param directory: Environment or game name
        ...
        :param files_list: List of two text files that contain a list of all images and landmarks
        Example: files_list = ['../Data-Pancreas/filenames/image_files.txt', '../Data-Pancreas/filenames/EDT_files.txt', '../Data-Pancreas/filenames/GDT_files.txt', '../Data-Pancreas/filenames/landmark_files.txt']

        :param landmark_ids: TODO: What is this?

        :param screen_dims: Shape of the frame cropped from the image to feed it to dqn (d,w,h)
            Default is (27,27,27)

        :param history_length: Consider lost of lives as end of episode (useful for training)

        :param max_num_frames: Maximum number of frames (steps) per episode.
        """
        super(MedicalImagesEnv, self).__init__()
        self.agents = agents
        ...
        self.saveGif = saveGif                      # Option to save display as gif
        self.saveVideo = saveVideo
        self.task = task                            # Training flag
        self.screen_dims = screen_dims              # Image dimensions (2D/3D)
        self.dims = len(self.screen_dims)           # 2D/3D
        ...
        with _ALE_LOCK:
            ...
            if isinstance(viz, six.string_types):  # Check if viz is a string
                ...
                viz = 0
            if isinstance(viz, int):               # Check if viz is an integer
                viz = float(viz)
            self.viz = viz
        ...
        self.action_space = spaces.Discrete(6)  # TODO: Change number of actions here
        self.actions = self.action_space.n
        self.observation_space = spaces.Box(low = 0, high = 255, shape = self.screen_dims, dtype = np.uint8)
        ...
        # Add data from DataLoader
        if file_type == "pancreas":
            self.files = DataPancreas(files_list, returnLandmarks, self.agents)

        # elif file_type == "rh":
        #     self.files = DataRH(files_list, returnLandmarks, self.agents)
        ...
        # Reset buffer, terminal, counters, and initialize new_random_game
    ...
    def new_random_game(self, fixed_spawn=None):
        """
            Load image, set dimensions, randomize start point, init _screen, q-vals, calculate distance to goal.
            # TODO: Start point should not be randomized!
        """
        self.terminal = [False] * self.agents
        self.viewer = None

        # Sample a new image
        self._image, self._EDT, self._GDT, self._landmarks, self.filepath, self.spacing = next(self.sampled_files)
        self._target_loc = self._landmarks[1] # End-point (F-2)
        ...
        if fixed_spawn is None:
            # Select random starting point # TODO: This needs to be changed to the starting point F-1!!
            ...
            # TODO: Change this to the (x,y,z)-coordinates of starting point F-1
            x = np.random.randint(
                    skip_thickness[0],
    ...
    def calcDistance(self, points1, points2, spacing=(1, 1, 1)):
        """
            ...
            # TODO: This needs to be changed to accommodate the EDT and GDT (look in RL_sandbox).
            # TODO: Input the distance field images from the DataLoader.
        """
    ...
    def _calc_reward(self, current_loc, next_loc, agent):
        """
            ...
            # TODO: This needs to be changed to accommodate the EDT and GDT (look in RL_sandbox).
            # TODO: Use the EDT and GDT values computed with the calcDistance method.
        """
    ...
    def step(self, act, q_values, isOver):
        ...
        :param terminal (boolean):
            ...
            all) tasks are divided up into well-defined episodes, and terminal
        ...
        # Movement of agent i
        # TODO: Change to accommodate 26 actions?
    ...
    def getBestLocation(self):
        """
            Get best location with best q-value from last for locations stored in history.
        """
    ...
    def _current_state(self):
        """
            Crop image data around current location to update what network sees, update rectangle.
            :return: new state
        """
    ...
    #         # Draw a box around the agent - what the network sees ROI - correct location if image is flipped
...
#     """
#         Class used when not training (wrapper for MedicalImagesEnv).
#     """
#
